...
pub use winit::event::{
    AxisId, DeviceEvent, DeviceId, ElementState, KeyEvent, Modifiers, MouseButton,
...
pub use winit::keyboard::{Key, NamedKey, SmolStr};
use winit::window::Window as WinitWindow;

use crate::{Application, MaverickOS};
...
pub(crate) struct Surface<A: Application>(
    Arc<WinitWindow>,
    &'static dyn Handle,
    Option<A::Renderer<'static>>,
);
impl<A: Application> Surface<A> {
    ...
    pub fn new(window: WinitWindow, context: &Context) -> Self {
        ...
        let handle: &'static dyn Handle =
            unsafe { std::mem::transmute::<&dyn Handle, &'static dyn Handle>(&*window) };
    ...
    pub fn resurface(&mut self, context: &Context) {
        self.2 = Some(A::Renderer::new(context, self.1));
    }
    pub fn request_redraw(&mut self) {
        self.0.request_redraw()
    }
    pub fn as_mut(&mut self) -> Option<&mut A::Renderer<'static>> {
        self.2.as_mut()
    }
}
...
impl<A: Application> ApplicationHandler for Window<A> {
    fn new_events(&mut self, _event_loop: &ActiveEventLoop, cause: StartCause) {
        if let Some(maverick) = self.0.as_mut()
            && let StartCause::ResumeTimeReached { .. } = cause
        {
    ...
    fn device_event(
        ...
        if let Some(maverick) = self.0.as_mut() {
            maverick
                .app
                .on_input(&mut maverick.context, Input::Device { device_id, event });
        }
    }
        ...
        match &mut self.0 {
            ...
            none => {
                let window = event_loop
                    .create_window(WinitWindow::default_attributes().with_title("orange"))
                    .unwrap();
            ...
            let event = match event {
                ...
                WindowEvent::RedrawRequested => {
                    ...
                    if let Some(surface) = maverick.surface.as_mut() {
                        ...
                        log::warn!("Redraw Requested Without A Valid Surface");
                    }
                    return;
                }
                ...
                WindowEvent::Resized(size) => {
                    ...
                    if let Some(surface) = maverick.surface.as_mut() {
                        ...
                        log::warn!("Resize Requested Without A Valid Surface");
                    }
                    Input::Resized
                }
                ...
                WindowEvent::DoubleTapGesture { device_id } => {
                    Input::DoubleTapGesture { device_id }
                }
                WindowEvent::RotationGesture {
                    device_id,
                    delta,
                    phase,
                } => Input::RotationGesture {
                    device_id,
                    delta,
                    phase,
                },
